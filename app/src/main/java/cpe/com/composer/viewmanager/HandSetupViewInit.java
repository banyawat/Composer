...
public class HandSetupViewInit {
    ...
    public HandSetupViewInit(ArrayList<ImageView> ImageViewID, InitialActivity ancestorActivity){
        ...
        normalShape = ancestorActivity.getResources().getDrawable(R.drawable.ic_panorama_fish_eye);
    ...
    private void setOnClickListener(final int index){
        fingerViews.get(index).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Toast.makeText(ancestorActivity.getApplicationContext(), String.valueOf(ancestorActivity.composerMovements.get(ancestorActivity.activeSlotPanel).getFingerValue(SIDE, fingerViewsId.indexOf(view.getId()))), Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    public void setInstrumentID(int keyID, int instrumentID){
        ancestorActivity.composerMovements.get(ancestorActivity.activeSlotPanel).setHandId(SIDE, fingerViewsId.indexOf(keyID), instrumentID);
    }
    ...
    public void refreshDrawable(){
        for (int i = 0; i < 5; i++) {
            if(ancestorActivity.composerMovements.get(ancestorActivity.activeSlotPanel).getFingerValue(SIDE, i)==-1)
    ...
    public void setActiveSlotPanel(int position){
        ancestorActivity.activeSlotPanel=position;
        SIDE=false;
        refreshDrawable();
    }
        ...
        public boolean onDrag(View v, DragEvent event) {
            ...
            switch (action) {
                ...
                case DragEvent.ACTION_DRAG_EXITED:
                    inside = false;
                    if(ancestorActivity.composerMovements.get(ancestorActivity.activeSlotPanel).getFingerValue(SIDE, fingerViewsId.indexOf(v.getId()))==-1) {
